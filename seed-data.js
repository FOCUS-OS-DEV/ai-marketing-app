// Run this script with Node.js to seed the database
// node seed-data.js

const { createClient } = require('@supabase/supabase-js');

const supabaseUrl = 'https://nzedphikmecnfzathwii.supabase.co';
const supabaseKey = 'sb_publishable_r0XkDfTek4loJoGr4kwElw_ReYLZ4h1';
...
const beginnersModules = [
  ...
  {
    id: 3,
    name: 'כלי AI לשיווק (מבוסס AI Ready)',
    color: '#8b5cf6',
    hours: 32,
    expanded: false,
    order: 3,
...
const proModules = [
  {
    id: 1,
    name: 'מבוא ל-AI ו-Prompt Engineering',
    color: '#1a1a2e',
    hours: 4,
    expanded: false,
    order: 1,
  ...
  {
    id: 6,
    name: 'AI Limitations, Ethics & Privacy',
    color: '#ef4444',
    hours: 4,
    expanded: false,
    order: 6,
...
async function seedDatabase() {
  ...
  try {
    // Seed beginners_modules
    console.log('Seeding beginners_modules...');
    const { error: begModErr } = await supabase
      .from('beginners_modules')
      .upsert({ id: 'main', data: beginnersModules });
    if (begModErr) throw begModErr;
    console.log('✓ beginners_modules seeded');

    // Seed pro_modules
    console.log('Seeding pro_modules...');
    const { error: proModErr } = await supabase
      .from('pro_modules')
      .upsert({ id: 'main', data: proModules });
    if (proModErr) throw proModErr;
    console.log('✓ pro_modules seeded');

    // Seed beginners_tasks
    console.log('Seeding beginners_tasks...');
    const { error: begTaskErr } = await supabase
      .from('beginners_tasks')
      .upsert({ id: 'main', data: beginnersTasks });
    if (begTaskErr) throw begTaskErr;
    console.log('✓ beginners_tasks seeded');

    // Seed pro_tasks
    console.log('Seeding pro_tasks...');
    const { error: proTaskErr } = await supabase
      .from('pro_tasks')
      .upsert({ id: 'main', data: proTasks });
    if (proTaskErr) throw proTaskErr;
    console.log('✓ pro_tasks seeded');

    // Seed beginners_config (with audiences)
    console.log('Seeding beginners_config...');
    const { error: begConfErr } = await supabase
      .from('beginners_config')
      .upsert({ id: 'main', data: { audiences: beginnersAudiences } });
    if (begConfErr) throw begConfErr;
    console.log('✓ beginners_config seeded');

    // Seed pro_audiences
    console.log('Seeding pro_audiences...');
    const { error: proAudErr } = await supabase
      .from('pro_audiences')
      .upsert({ id: 'main', data: proAudiences });
    if (proAudErr) throw proAudErr;
    console.log('✓ pro_audiences seeded');

    // Seed beginners_content
    console.log('Seeding beginners_content...');
    const { error: begContErr } = await supabase
      .from('beginners_content')
      .upsert({
        id: 'main',
        vision: 'תוכנית הכשרה מקיפה לשיווק דיגיטלי עם AI',
...
• שקיפות מלאה בנתוני השמה`
    ...
    if (begContErr) throw begContErr;
    console.log('✓ beginners_content seeded');

    // Seed pro_content
    console.log('Seeding pro_content...');
    const { error: proContErr } = await supabase
      .from('pro_content')
      .upsert({
        id: 'main',
        vision: `תוכנית Pro למשווקים מנוסים
...
• גישה לכלי AI מתקדמים`
    ...
    if (proContErr) throw proContErr;
    console.log('✓ pro_content seeded');
